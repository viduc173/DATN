...
public class VehicleAudio : MonoBehaviour
	...
	void Update ()
		...
		// Bỏ qua frame audio nếu VehicleController chưa được enable (cachedRigidbody = null).
		// Tránh NullReferenceException khi xe bị disable lúc scene start (vd: MatchWaitTime countdown).
		if (m_vehicle.cachedRigidbody == null)
			{
			StopAllAudioSources();
			return;
			}
	...
	void DoWindAudio ()
		{
		float windRatio = Mathf.InverseLerp(wind.minSpeed, wind.maxSpeed, m_vehicle.cachedRigidbody.linearVelocity.magnitude);
