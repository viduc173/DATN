...
public class WheelData
	...
	public PhysicsMaterial lastPhysicMaterial = new PhysicsMaterial();	// A new physic material ensures the first iteration to match the changes.
...
public class VehicleController : MonoBehaviour
	...
	void OnEnable ()
		...
		// Unity 6 enables automatic CoM/inertia by default, which silently ignores
		// the manual `centerOfMass` assignment in ConfigureCenterOfMass() below.
		// Disable both so EVP5's parametric CoM actually applies.
		...
		m_lastImpactedMaterial = new PhysicsMaterial();	// A new reference to ensure cache missmatch at the first query
		}
	...
	void FixedUpdate ()
		...
		if (m_referenceBody != null) currentVelocity -= m_referenceBody.linearVelocity;
		...
		float sqrVelocity = m_rigidbody.linearVelocity.sqrMagnitude;
		Vector3 normalizedVelocity = m_rigidbody.linearVelocity.normalized;
	...
	float FixSteerAngle (WheelData wd, float inputSteerAngle)
 	...
 	void UpdateSuspension (WheelData wd)
		...
		wd.hit.point += m_rigidbody.linearVelocity * Time.deltaTime;
	...
	void UpdateGroundMaterialCached (PhysicsMaterial colliderMaterial, ref PhysicsMaterial cachedMaterial, ref GroundMaterial groundMaterial)
		{
	...
	PhysicsMaterial m_lastImpactedMaterial;
